...
typedef struct logindatalink/*the first is wrongnumber ,the second one is password ,the third one is date */
{
	struct logindata;
	struct logindatalink *next;
}logindatalink;/*学艺不精啊！！！！！*/
...
while(1)
{
	...
	if(judge==0)
	    ...
	    printf("enter 1 to display all scores ,enter 0 to quit.\n");
	while(1)
	{
		scanf("%d",&select);
		if(select==1||select==2||select==0)
		{
			if(judge==0)
				break;
	        else if(judge==1&&(select==1||select==0))
			{
