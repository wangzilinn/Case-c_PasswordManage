...
int efficiencymanagementsystem()
{
int all[101];/*all[0] save the number of days*/
int select;
int newscore;
memset(all,0,sizeof(all));/*fill the array with 0*/
sentence week[7];
int count=0;
FILE *fp;
...
printf("loading....\n");
fread(all,sizeof(all[0]),101,fp);/*read*/
count=0;
fclose(fp);
decoratline(2);
printf("			Efficiency Management System");
printf("		You have been studying for %d days",all[0]);
decoratline(7);/*while ,but if function 1 has been run ,then just function 2 is permit*/
printf("enter 1 to set today's score,enter 2 to display all scores，enter 0 to quit.\n");
while(1)
{
	scanf("%d",&select);
...
switch(select)
{
	case(1):
		...
		if(count==100)/*when array is full*/
		{
            for(count=1;all[count]!=NULL;count++)
                all[count]=0;
            count=1;
		}
		all[count]=newscore;
		...
		count=0;
		fwrite(all,sizeof(all[0]),101,fp);
        break;
    case(2):
    case(0):return(0);
}
